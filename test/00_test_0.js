...
describe("Nix", function () {
  ...
  beforeEach(async function () {
    ...
    const mint0Tx = await nftA.mint(data.maker0);
    const mint1Tx = await nftA.mint(data.maker0);
    const mint2Tx = await nftA.mint(data.maker0);
    const mint3Tx = await nftA.mint(data.taker0);
    const mint4Tx = await nftA.mint(data.taker0);
    const mint5Tx = await nftA.mint(data.taker0);
    if (DETAILS > 0) {
    ...
    const approve0Tx = await nftA.connect(data.maker0Signer).setApprovalForAll(nix.address, true);
    const approve1Tx = await nftA.connect(data.maker1Signer).setApprovalForAll(nix.address, true);
    const approve2Tx = await nftA.connect(data.taker0Signer).setApprovalForAll(nix.address, true);
    const approve3Tx = await nftA.connect(data.taker1Signer).setApprovalForAll(nix.address, true);
    if (DETAILS > 0) {
  ...
  it("0. Maker BuyAny Test", async function () {
    ...
    const makerAddOrder1Tx = await nix.connect(data.maker0Signer).makerAddOrder(ZERO_ADDRESS, nftA.address, [ 3, 4, 5 ], ethers.utils.parseEther("11"), ORDERTYPE.BUYANY, 0, 2, { value: ethers.utils.parseEther("0.000000001") });
    ...
    const makerAddOrder2Tx = await nix.connect(data.maker0Signer).makerAddOrder(ZERO_ADDRESS, nftA.address, [ ], ethers.utils.parseEther("0.0011"), ORDERTYPE.BUYANY, expiry2, 2, { value: ethers.utils.parseEther("0.000000001") });
    ...
    const takerExecuteOrder1Tx = await nix.connect(data.taker0Signer).takerExecuteOrder(0, [ 3, 5 ], ethers.utils.parseEther("22"), { value: ethers.utils.parseEther("0.000000001") });
    await data.printEvents("Taker Sold #3 against BuyAny Max 2 NFTA:{3|4|5} for 11e" , await takerExecuteOrder1Tx.wait());
    const takerExecuteOrder2Tx = await nix.connect(data.taker0Signer).takerExecuteOrder(1, [ 4 ], ethers.utils.parseEther("0.0011"), { value: ethers.utils.parseEther("0.000000001") });
  ...
  it("1. Maker SellAny Test", async function () {
    console.log("    ==== Maker Add Orders === ");
    const makerAddOrder1Tx = await nix.connect(data.maker0Signer).makerAddOrder(ZERO_ADDRESS, nftA.address, [ 0, 1, 2 ], ethers.utils.parseEther("12.3456"), ORDERTYPE.SELLANY, 0, 1, { value: ethers.utils.parseEther("0.000000001") });
    await data.printEvents("Maker Added Order #0 - SellAny NFTA:{0|1|2} for 12.3456e", await makerAddOrder1Tx.wait());
    const expiry2 = parseInt(new Date() / 1000) + (60 * 60 * 24);
    const makerAddOrder2Tx = await nix.connect(data.maker0Signer).makerAddOrder(ZERO_ADDRESS, nftA.address, [ ], ethers.utils.parseEther("1.23456"), ORDERTYPE.SELLANY, expiry2, 1, { value: ethers.utils.parseEther("0.000000001") });
    await data.printEvents("Maker Added Order #1 - SellAny NFTA:* for 1.23456e", await makerAddOrder2Tx.wait());
    await data.printState("After Maker Added Orders");

    console.log("    ==== Taker Execute Against Orders === ");
    const takerExecuteOrder1Tx = await nix.connect(data.taker0Signer).takerExecuteOrder(0, [ 1 ], ethers.utils.parseEther("12.3456"), { value: ethers.utils.parseEther("0.000000001") });
    await data.printEvents("Taker Bought #1 against SellAny NFTA:{0|1|2} for 12.3456e" , await takerExecuteOrder1Tx.wait());
    const takerExecuteOrder2Tx = await nix.connect(data.taker0Signer).takerExecuteOrder(1, [ 2 ], ethers.utils.parseEther("1.23456"), { value: ethers.utils.parseEther("0.000000001") });
  ...
  it("2. Maker BuyAll Test", async function () {
    console.log("    ==== Maker Add Orders === ");
    const makerAddOrder1Tx = await nix.connect(data.maker0Signer).makerAddOrder(ZERO_ADDRESS, nftA.address, [ 3, 4, 5 ], ethers.utils.parseEther("12.3456"), ORDERTYPE.BUYALL, 0, 1, { value: ethers.utils.parseEther("0.000000001") });
    await data.printEvents("Maker Added Order #0 - BuyAll NFTA:{3&4&5} for 12.3456e", await makerAddOrder1Tx.wait());
    await data.printState("After Maker Added Orders");

    console.log("    ==== Taker Execute Against Orders === ");
    const takerExecuteOrder1Tx = await nix.connect(data.taker0Signer).takerExecuteOrder(0, [ 3, 4, 5 ], ethers.utils.parseEther("12.3456"), { value: ethers.utils.parseEther("0.000000001") });
  ...
  it("3. Maker SellAll Test", async function () {
    console.log("    ==== Maker Add Orders === ");
    const makerAddOrder1Tx = await nix.connect(data.maker0Signer).makerAddOrder(ZERO_ADDRESS, nftA.address, [ 0, 1, 2 ], ethers.utils.parseEther("12.3456"), ORDERTYPE.SELLALL, 0, 1, { value: ethers.utils.parseEther("0.000000001") });
    await data.printEvents("Maker Added Order #0 - SellAll NFTA:{0&1&2} for 12.3456e", await makerAddOrder1Tx.wait());
    await data.printState("After Maker Added Orders");

    console.log("    ==== Taker Execute Against Orders === ");
    const takerExecuteOrder1Tx = await nix.connect(data.taker0Signer).takerExecuteOrder(0, [ 0, 1, 2 ], ethers.utils.parseEther("12.3456"), { value: ethers.utils.parseEther("0.000000001") });
...
// if (false) {
//   const exchangeTx = await nix.connect(data.maker0Signer).exchange(nftA.address, 1, maker1);
